...
def initial_eta(flow_train,l0):
    ...
    Theta = np.ones((Num_st,Num_st,L)) * 1e-2 
    l = l0 #layer 1
    for q in range(0,Q):
        q
...
def log_gamma_i_z(gamma_i,Theta,Z,i):
    ...
    for j in range(0,Num_st): #station
        for r1 in range(0,R):
            ...
            if theta_rr1 == 0:
                theta_rr1 = theta_rr1 + math.exp(-6)  
            if theta_rr1 == 1:
                theta_rr1 = theta_rr1 - math.exp(-6)  
...
for num_c in range(0,R):     
    num_c
    orignal_com = pandas.read_csv('community number_attributes.csv',header=None) 
    com = orignal_com.values
    Num_c = np.argwhere(com==num_c)[:,0]
    flow_train_c = flow_train[:,Num_c,:,:] 
...
All = 100 # total iteration number
...
print('MAE = ', MAE) #MAE =  5.137234609416091
print('RMAE = ', RMAE) #RMAE =  0.1848854205162066

#Two-step forecast
MAE = abs(hat_flow2 - flow_test).mean() 
RMAE = abs(hat_flow2 - flow_test).sum() / flow_test.sum() 
print('MAE = ', MAE) #MAE = 6.650612759339639
print('RMAE = ', RMAE) #RMAE = 0.23935082397194962

#Three-step forecast
MAE = abs(hat_flow3 - flow_test).mean() 
RMAE = abs(hat_flow3 - flow_test).sum() / flow_test.sum() 
print('MAE = ', MAE) #MAE = 7.918688037274515 
print('RMAE = ', RMAE) #RMAE =  0.28498795149917444
...
print('l = 0, MAE = ', MAE) #MAE = 5.26192053290166
print('l = 0, RMAE = ', RMAE) #RMAE = 0.18703552972664847
l = 1
MAE = abs(hat_flow1[:,:,:,l] - flow_test[:,:,:,l] ).mean() 
RMAE = abs(hat_flow1[:,:,:,l]  - flow_test[:,:,:,l] ).sum() / flow_test[:,:,:,l].sum() 
print('l = 1, MAE = ', MAE) #MAE =  5.012548685930522
print('l = 1, RMAE = ', RMAE) #RMAE =  0.18268089456166192


l = 0
MAE = abs(hat_flow2[:,:,:,l] - flow_test[:,:,:,l] ).mean() 
RMAE = abs(hat_flow2[:,:,:,l]  - flow_test[:,:,:,l] ).sum() / flow_test[:,:,:,l].sum() 
print('l = 0, MAE = ', MAE) #MAE = 6.891837455647291
print('l = 0, RMAE = ', RMAE) #RMAE = 0.24497110156776267
l = 1
MAE = abs(hat_flow2[:,:,:,l] - flow_test[:,:,:,l] ).mean() 
RMAE = abs(hat_flow2[:,:,:,l]  - flow_test[:,:,:,l] ).sum() / flow_test[:,:,:,l].sum() 
print('l = 1, MAE = ', MAE) #MAE = 6.409388063031986
print('l = 1, RMAE = ', RMAE) #RMAE = 0.2335883037373754


l = 0
MAE = abs(hat_flow3[:,:,:,l] - flow_test[:,:,:,l] ).mean() 
RMAE = abs(hat_flow3[:,:,:,l]  - flow_test[:,:,:,l] ).sum() / flow_test[:,:,:,l].sum() 
print('l = 0, MAE = ', MAE) #MAE = 8.26935524875269
print('l = 0, RMAE = ', RMAE) #RMAE = 0.29393511927391286
l = 1
MAE = abs(hat_flow3[:,:,:,l] - flow_test[:,:,:,l] ).mean() 
RMAE = abs(hat_flow3[:,:,:,l]  - flow_test[:,:,:,l] ).sum() / flow_test[:,:,:,l].sum() 
print('l = 1, MAE = ', MAE) #MAE = 7.568020825796341
print('l = 1, RMAE = ', RMAE) #RMAE = 0.27581434139449396
