...
def model_proposed_delta(flow_train,st_info,delta):
    ...
    for numa in range(0,All-1):
        ...
        for i in range(0,Num_st):
            ...
            for j in range(0,Num_st):
                ...
                log_Pgamma_i, Alpha_i = log_posterior_gamma_i(i,gamma_i,flow_train,Mu,delta)
...
def Alphi_i_initialization(i,gamma_i,flow_train,Mu,delta):
    import numpy as np
...
def log_posterior_gamma_i(i,gamma_i,flow_train,Mu,delta):
    import numpy as np
    ...
    Alpha_i = Alphi_i_initialization(i,gamma_i,flow_train,Mu,delta)
    for q in range(0,Q):
        for l in range(0,L):
            Oi = np.argwhere((gamma_i==1))[:,0]
            flow_train_Oi = flow_train[:,Oi,:,:]
...
def model_proposed_test(flow_test,flow_test_CIF,All_Mu,All_Alpha,delta):
    import numpy as np
    Num_st = 20 #nodes
    Num_time = 100 #time points
    Num_month_train = 50 #samples
